...
CMDs[#CMDs + 1] = {NAME = 'invispartesp', DESC = 'View all invisible parts WITHOUT making them visible'}
...
CMDs[#CMDs + 1] = {NAME = 'gototool / gotoitems [tools HANDLE name]', DESC = 'Teleports you to a tool or multiple'}
...
function invisesp()
	for i, v in pairs(workspace:GetDescendants()) do
		if v:IsA("BasePart") and v.Transparency >= 0.7 then
			local a = Instance.new("BoxHandleAdornment")
			a.Size = v.Size
			a.Name = v.Name .. " _feinious?"
...
function uninvisesp()
	for i, v in pairs(workspace:GetDescendants()) do
		if v:IsA("BoxHandleAdornment") and string.find(v.Name, "_feinious?") then
			v:Destroy()
		end
	end
end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/jannnerb/my-own-inf-yield-update/refs/heads/main/infiniteyield.lua'))()")
	end
end)
...
addcmd('invispartesp',{},function(args, speaker)
	invisesp()
end)

addcmd('uninvispartesp',{},function(args, speaker)
	uninvisesp()
end)
...
addcmd('gototool',{'gotoitem'},function(args, speaker)
	for i, v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1):lower() and v:IsA("Tool") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
				wait(.1)
			end
			wait(gotopartDelay)
			getRoot(speaker.Character).CFrame = v.Handle.CFrame
		end
	end
end)
